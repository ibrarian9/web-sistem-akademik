<div class="space-y-6 font-sans">
    ...
    >
        <x-slot:actions>
            <!-- Tombol CTA Buka Kasir Pembayaran -->
            <x-button variant="primary" size="md" icon="credit-card" href="{{ route('finance.input-pembayaran') }}" wire:navigate>
    ...
    <!-- Tab Switcher: Matriks Tagihan Terpadu (SPP 6 Bulan & Kategori Tunggakan) vs Matriks Bulanan vs Tabel Ringkas -->
    <div class="flex items-center justify-between flex-wrap gap-3 p-1.5 bg-stone-100 border border-stone-200 rounded-2xl shadow-2xs">
        <div class="flex items-center gap-1.5 w-full sm:w-auto flex-wrap">
            <button 
                type="button" 
                wire:click="setViewTab('spp_matrix')" 
                class="flex-1 sm:flex-initial px-4 py-2.5 text-xs font-bold rounded-xl transition flex items-center justify-center gap-2 cursor-pointer {{ $activeViewTab === 'spp_matrix' ? 'bg-white text-emerald-800 shadow-xs border border-stone-200/80 font-black ring-1 ring-emerald-600/10' : 'text-stone-600 hover:text-stone-900 hover:bg-white/60' }}"
            >
                <x-lucide-table-2 class="w-4 h-4" />
                <span>Matriks Tagihan (SPP 6 Bulan dan Kategori Tunggakan)</span>
            </button>
            <button 
                type="button" 
                wire:click="setViewTab('lengkap')" 
                class="flex-1 sm:flex-initial px-4 py-2.5 text-xs font-bold rounded-xl transition flex items-center justify-center gap-2 cursor-pointer {{ $activeViewTab === 'lengkap' ? 'bg-white text-emerald-800 shadow-xs border border-stone-200/80 font-black ring-1 ring-emerald-600/10' : 'text-stone-600 hover:text-stone-900 hover:bg-white/60' }}"
            >
                <x-lucide-calendar-range class="w-4 h-4" />
                <span>Matriks Bulanan per Kategori</span>
            </button>
            <button 
                type="button" 
            ...
            >
                ...
                <span>Daftar Ringkas Siswa</span>
            </button>
        </div>

        <div class="hidden lg:flex items-center gap-2 text-xs text-stone-600 font-medium px-2">
            @if ($activeViewTab === 'spp_matrix')
                <span>Menampilkan leger gabungan: SPP terbatas 6 bulan dipadukan dengan seluruh kategori tagihan non-SPP</span>
            @elseif ($activeViewTab === 'lengkap')
                <span>Menampilkan rincian status pelunasan seluruh jenis tagihan pada bulan {{ $selectedBulan }}</span>
            @else
                <span>Menampilkan ringkasan total tagihan & tunggakan akumulasi per siswa</span>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white border border-stone-200 rounded-2xl p-3.5 sm:p-6 shadow-xs space-y-4">
        @if ($activeViewTab === 'spp_matrix')
            @include('livewire.finance.overview-pembayaran.partials.table-spp-matrix')
        @elseif ($activeViewTab === 'lengkap')
            @include('livewire.finance.overview-pembayaran.partials.table-detailed-matrix')
        @else
            <!-- Filter & Search Controls (Tabel Default) -->
                    ...
                    @forelse ($siswas as $item)
                        <tr class="hover:bg-emerald-50/40 transition">
                            ...
                            <td class="p-3.5 border-r border-stone-200">
                                ...
                                <div class="text-[10px] text-stone-600 font-mono font-medium">NIS: {{ $item['nis'] }}</div>
                            </td>
                            <!-- Class -->
                            <td class="p-3.5 border-r border-stone-200">
                            ...
                            <td class="p-3.5 text-center border-r border-stone-200">
                                @if ($item['total_tagihan_count'] > 0)
                                    ...
                                    <div class="text-[10px] text-stone-600 font-medium">
                            ...
                            <td class="p-3.5 text-right border-r border-stone-200">
                                <div class="text-[11px] text-stone-600 font-medium">Tagihan: Rp {{ number_format($item['total_nominal'], 0, ',', '.') }}</div>
                                ...
                                @if (!empty($item['sisa_mendatang']) && $item['sisa_mendatang'] > 0)
                                    <div class="text-[10px] text-stone-500 font-medium mt-0.5">Mendatang: Rp {{ number_format($item['sisa_mendatang'], 0, ',', '.') }}</div>
                                @endif
                            </td>
                            <!-- Custom Status Badge -->
                            <td class="p-3.5 text-center border-r border-stone-200">
    ...
    <!-- Modal: Matriks Tagihan 12 Bulan Per-Murid -->
    @if ($showSiswaMatrixModal && $siswaMatrixData)
        @include('livewire.finance.overview-pembayaran.partials.modal-siswa-matrix')
    @endif
</div>
